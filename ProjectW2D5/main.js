const movies = [
    ...
    - Als ik een ander filter aanklik gaat het andere filter dus weer uit. (Hence de **radiobutton**, in tegenstelling tot een checkbox).*/

 // array maken met alle titels van de films
      const allNames = movies.map((item) => {
        return item.Title
      })
...
/*- Als gebruiker wil ik kunnen filteren op de categorie **nieuwste films**: van de laatste 5 jaar, dat betekent 2014 of ouder.*/

// array maken met alleen nieuwste films
...
// Als gebruiker wil ik kunnen filters op films met **"Avengers"** in de titel.*/
// array maken met Avengers in titel

      const avengersMovies = movies.filter((item) => {
...
// Array maken met X-Men in de titel
      const xmenMovies = movies.filter((item) => {
...
// Array maken met Princess in de titel
      const princessMovies = movies.filter((item) => {
      return item.Title.includes ("Princess")
    })
    console.log(princessMovies);    


// Als gebruiker wil ik kunnen filteren op films met **"Batman"** in de titel.
// Array maken met Batman in de titel
      const batmanMovies = movies.filter((item) => {
...
/* Als gebruiker kan ik op de poster van de film klikken, waardoor ik naar de juiste IMDB pagina wordt gebracht.
    ...
    Pas de URL van IMDB aan en plak het juiste ID erachter dat je terugvindt per film in ons databeestje.*/

      const posterClick = movies.filter((item) => {
        return item.imdbID = "https://www.imdb.com/title/" + item.imdbID

      })
      console.log (posterClick);

/*- Als gebruiker wil ik een lijst kunnen zien van de beschikbare films **met de poster van de film.**
- Elke film in de data heeft een link naar de poster (een image) (die is dus al voor je beschikbaar, je hoeft ze alleen nog maar te vinden, met een arrayfunctie wellicht).
- Hoe kun je gemakkelijk een lijst html elementen maken vanuit een array met posterlinks? Met een loop natuurlijk!  [StackOverflow to the rescue](https://stackoverflow.com/questions/46141450/create-li-from-loop-through-array-and-display-to-html-as-a-list)
- 🚀 Wil je je pagina graag stylen met [CSS-grid](https://css-tricks.com/snippets/css/complete-guide-grid/)? Ja! Graag!
- 🚀 Wil je je pagina graag stylen met [flex-box](https://css-tricks.com/snippets/css/a-guide-to-flexbox/)? Ja! Graag!*/

const postersUrls = movies.map((item) => {
  return item.Poster
})
var postersUl = '<ul>';

postersUrls.forEach((posterUrl) => {
  postersUl += `<li> <img src= ${posterUrl} heigth= "100" width="80" /></li>`
  console.log (postersUl);
}); 

postersUl += '</ul>';
document.getElementById('movieList').innerHTML = postersUl;

// Functies voor radiobutton

function newMoviesClick(){ 
   getNewMovies = document.getElementById("newMovies");
  return newMovies
}
document.getElementById('movielist').postersUl = newMoviesClick;


function avanagerMoviesClick() {
  document.getElementById("avengersMovies").checked = true;
  return avengersMovies
}
function xmenMoviesClick() {
  document.getElementById("xmenMovies").checked = true;
  return xmenMovies
}
function princessMoviesClick() {
  document.getElementById("princessMovies").checked = true;
  return princessMovies
}
function batmanMoviesClick() {
  document.getElementById("batmanMovies").checked = true;
  return batmanMovies
}